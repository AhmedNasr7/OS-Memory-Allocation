...
class Memory:
    def __init__(self, memory_size):
        ...
        self.memory_contents = [
            ["old_process", self.color_from_name("old_process"), memory_size]
        ]
            ...
            for j in range(len(output_list)):
                if output_list[j][0] == "hole":
                    if output_list[j][2] >= segments_list[i][1]:
                        ...
                        output_list.insert(
                            j, [segments_list[i][0], color, segments_list[i][1]]
                        )
                        if output_list[j + 1][2] == 0:
                            output_list.pop(j + 1)
                        segements = segements + 1
                        break
        assert segements == len(
            segments_list
        ), "Not Enough memory to allocate this process"
        ...
        for segment in segments:
            ...
            if left_over > 0:
                ...
                self.memory_contents.insert(
                    hole_index,
                    [segment_name, self.color_from_name(process_name), segment_size],
                )
            else:
                self.memory_contents[hole_index] = [
                    segment_name,
                    self.color_from_name(process_name),
                    segment_size,
                ]
    ...
    def worst_fit(self, Segments, process_name):
        ...
        for segment in Segments:
            ...
            for i in range(len(self.memory_contents)):
                if (
                    self.memory_contents[i][2] >= self.memory_contents[Max][2]
                    and self.memory_contents[i][0] == "hole"
                ):
                    Max = i
                    Flag = True

            assert (
                Flag and self.memory_contents[Max][2] >= segment[1]
            ), "Not Enough memory to allocate this process"
            ...
            if self.memory_contents[Max + 1][2] == segment[1]:
                ...
                self.memory_contents[Max + 1][2] = (
                    self.memory_contents[Max + 1][2] - segment[1]
                )
        ...
        while Flag:
            ...
            for i in range(len(self.memory_contents) - 1):
                if self.memory_contents[i][0] == self.memory_contents[i + 1][0]:
                    self.memory_contents[i][2] = (
                        self.memory_contents[i][2] + self.memory_contents[i + 1][2]
                    )
    ...
    def add_hole(self, starting_address, hole_size):
        assert (
            starting_address + hole_size <= self.memory_size
        ), "Hole Address or Size exceeds memory limits, please reassign their values"
        sum = 0
        for i in range(len(self.memory_contents)):
            sum = sum + self.memory_contents[i][2]
            if sum > starting_address:
                assert (
                    self.memory_contents[i][0] == "old_process"
                ), "Can't add a hole here"
                assert (
                    sum - (starting_address + hole_size) >= 0
                ), "Hole size excedded the limits"
                if sum - (starting_address + hole_size) == 0:
                    self.memory_contents[i][2] -= hole_size
                    self.memory_contents.insert(
                        i + 1, ["hole", self.color_from_name("hole"), hole_size]
                    )
                    break
                else:
                    self.memory_contents[i][2] = self.memory_contents[i][2] - (
                        sum - starting_address
                    )
                    self.memory_contents.insert(
                        i + 1, ["hole", self.color_from_name("hole"), hole_size]
                    )
                    self.memory_contents.insert(
                        ...
                        [
                            "old_process",
                            self.color_from_name("old_process"),
                            sum - (starting_address + hole_size),
                        ],
                    )
...
def main():
    # # Testing
    Segments = [["Code", 50], ["Data", 90], ["Stack", 15]]
